        ...
        try:
            ...
            if line:
                ...
                with data_lock:
                    ...
                    data["Voltage (V)"].append(float(values[0].split()[0]))
                    data["Current (mA)"].append(float(values[1].split()[0]))
                    data["Temperature 1 (C)"].append(float(values[2].split()[0]))
                    data["Temperature 2 (C)"].append(float(values[3].split()[0]))
...
fig, axes = plt.subplots(2, 2, figsize=(10, 8))  # Create a 2x2 grid for four graphs
axes = axes.flatten()
